<template>
  <div class="outbox">
    <div class="login">
      ...
      <a-button size="large" type="primary" class="btn-login" @click="submitForm">{{
...
const submitForm = async () => {
  ...
  if (Object.keys(login.errors).length === 0) {
    try {
      ymPoint("login_click_from_forget", "点击重置密码按钮");
    } catch (error) {
      console.log("埋点", error);
    }
    // 提交表单逻辑手机号+验证码登录
    try {
      const res = await services.user.forget(login.phone, login.code, md5(login.password));
      if (res.code === 0) {
        message.success(t("success", { msg: t("login.resetPassword") }));
        router.push({
          name: "login",
        });
      } else {
        message.error(res.message ? res.message : t("fail", { msg: t("login.resetPassword") }));
      }
    } catch (error) {
      message.error(t("fail", { msg: t("login.resetPassword") }));
    }
  }
};
//跳转到注册页面
