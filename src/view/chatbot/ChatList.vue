  ...
  <nav class="navbar">
    <div class="toolbar">
      <div class="addBtn" @click="addConversation">
        ...
        <div class="itemRight">{{ $t("chat.newChat") }}</div>
      </div>
    </div>
    <div class="list">
      <ul>
        ...
        >
          <div class="title" v-if="conversation.itemShow == 'normal'">
            <div class="itemLeft">
              <img class="itemIcon" :src="chatIcon" />
            </div>
            <div class="itemRight">
          ...
          <div class="title" v-if="conversation.itemShow == 'deleteShow'">
            ...
            <div class="itemRight">{{$t("confirmDelete")}}"{{ conversation.chatName || $t("chat.newChat") }}"</div>
          </div>
          <div class="title" v-if="conversation.itemShow == 'editShow'">
            ...
            <div class="itemRight">
              <input class="editInput" id="edit-input" type="text" v-model="conversation.chatName" placeholder="" />
            </div>
          </div>
...
const addConversation = () => {
  ...
  // let model = modelList.value[modelList.value.length - 1]
...
const deleteConversation = async (index) => {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      message.warning(res.message ? res.message : "删除会话接口请求失败");
    }
  } catch (error) {
    console.log(error);
    message.warning("删除会话接口调用失败");
  }
};
...
const editClick = (itemShow, index, status) => {
  ...
  if (status == "close") {
    conversations.value[index].chatName = oldName;
  } else if (status == "open") {
    oldName = conversations.value[index].chatName;
  } else if (status == "ok") {
    if (conversations.value[index].chatName && conversations.value[index].chatName != "") {
      const updateChat = async () => {
        ...
        try {
          const res = await services.chat.updateChat(
            conversations.value[index].chatName,
