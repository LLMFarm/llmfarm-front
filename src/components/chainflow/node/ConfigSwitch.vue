<template>
  <div class="config-container" @click="show">
    <span style="font-weight: 500;">
      Switch 配置
    </span>

    <a-modal v-model:visible="visible" title="条件路由配置" width="880px" :showCount="true">
      <div class="content">

        <a-button type="primary" style="font-size: 12px; width: 100px" @click="append">新增路由</a-button>

        <div class="section" v-for="(condition, index) in conditions" :key="index">
          <a-input v-model:value="condition.match"> </a-input>
          <NodeSelector v-model:value="condition.nodeId" :options="nodeOptions" style="width: 100%;" />
          <img :src="removeIcon" class="remove" @click="remove(index)" />
        </div>

        <div class="footer">
          <p>说明: 配置 switch 匹配逻辑 & 路由</p>
