...
const changeService = () => {
  flowNode.value.title = props.option.service || 'API';
}

const changeAPI = (apiValue) => {
  const list = currentService.value.list || [];
  const option = list.find(item => item.uri == apiValue);
  flowNode.value.desc = option ? option.name : 'API';
  if (option && option.name) {
    const request = props.option.request;
    for (const r of option.request) {
      request[r.key] = { type: 'string', nodeId: '', value: '' }
    }
  }
}
